...
def mask2bbox(attention_maps):
    ...
    for i in range(attention_maps.shape[0]):
        ...
        min_activate = 0.05 * max_activate
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        if 'resnet' in FLAGS.model_name:
            ...
            logits_to_updates = add_eval_summary(logits, labels, scope='/logits')
            ...
            logits_1, end_points_1 = network_fn(images, indices)
            topk_logits = end_points_1['topk_logits']

            attention_maps = tf.reduce_mean(end_points_1['attention_maps'], axis=-1, keepdims=True)
            # m = attention_maps.shape[1]
            # attention_maps = tf.reshape(attention_maps, [FLAGS.batch_size, -1])
            # attention_maps = tf.nn.softmax(attention_maps, axis=1)
            # attention_maps = tf.reshape(attention_maps, [-1, m, m, 1])
            ...
            logits_2, end_points_2 = network_fn(images, indices, reuse=True)

            logits = logits_1 + logits_2

            ens_logits = (0.4 * logits_1 + 0.6 * logits_2) #+ 0.1 * (topk_logits)
            ens_logits = tf.py_func(get_logits, [ens_logits], [tf.float32])
            ens_logits = tf.reshape(ens_logits, [FLAGS.batch_size, len(classes)])

            labels = tf.py_func(get_labels, [labels], [tf.int64])
            ...
            logits_3_to_updates = add_eval_summary(topk_logits, labels, scope='/topk_logits')
            logits_4_to_updates = add_eval_summary(ens_logits, labels, scope='/ens_logits')
        ...
        while True:
            ...
            eval_op.extend(list(logits_3_to_updates.values()))
            eval_op.extend(list(logits_4_to_updates.values()))
            ...
            # print(num_bboxes)
            bboxes = np.array(num_bboxes)
            with open('num_bboxes.pkl', 'wb') as f:
                pickle.dump(bboxes, f)
            with open('eval_logits.pkl', 'wb') as f:
                pickle.dump(eval_logits, f)

            with open('eval_labels.pkl', 'wb') as f:
